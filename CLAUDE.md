...
- **Vídeo do YouTube ou Instagram (opcional)** — `Exercicio.videoUrl` (string livre, limite 500 igual à `descricao`; nome do campo já era platform-agnostic, não precisou mudar), campo `VideoLinkField` (`components/video-link-field.tsx`, antes `YoutubeVideoField`/`youtube-video-field.tsx` — renomeado quando ganhou suporte a Instagram) renderizado logo abaixo da galeria (tanto em `exercicios/lista.tsx` quanto na tela de exercício-dentro-de-sessão do calendário).
