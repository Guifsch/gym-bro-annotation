...
- **Modelo simples e livre**: `Refeicao` tem `nome`, `dates: string[]` (YYYY-MM-DD, zero ou mais), `itens: { _id, nome }[]` (itens avulsos, fora de qualquer bloco), `blocos: { _id, nome, horario?, itens: {_id,nome}[] }[]` (subcategorias opcionais dentro da refeição, ex: refeição "Plano de terça" com blocos "Café da manhã · 8:00" e "Almoço · 12:00", cada um com seus próprios itens), `observacoes?`. Sem contagem de calorias/macros — é só uma lista organizável, por design (usuário não tinha um formato em mente, optei pelo mais flexível). `horario` é texto livre (sem picker nativo, sem validação de formato) — mesma filosofia de flexibilidade do resto da feature.
- **Sem limite de criação** (diferente de categorias/exercícios/treinos/timers): refeições são um log de uso (mais parecido com `Sessao`, que também não tem limite) e não um catálogo/definição — a natureza é acumular ao longo do tempo, um limite baixo inviabilizaria o uso real. Limites defensivos de array continuam existindo (`dates` até 60, `blocos` até 20, `itens` até 30 por lista).
...
- **Itens (tanto avulsos quanto dentro de bloco) e blocos**: cada `PATCH` manda o array inteiro correspondente (`itens` e/ou `blocos`) — sem endpoints dedicados de add/remove, mesmo padrão simples usado pra `exercicioIds` do Treino. Editar item de um bloco = reconstruir o array `blocos` inteiro no cliente (`toBlocoParams`/`toItemParams` em `[refeicaoId].tsx`) mudando só o bloco afetado.
- **UI: separação visual item-existente vs. campo-de-adicionar** — bug real corrigido: listas e o form de "adicionar novo" usavam o mesmo componente `Card`, ficando visualmente idênticos e confundindo o usuário sobre o que já tinha sido salvo. Fix: rótulo próprio acima de cada bloco de input ("Adicionar item avulso novo", "Adicionar item a este bloco", "Novo bloco") e, na lista de itens avulsos, uma borda superior (`addItemSection`) separando claramente do que já existe.
...
- **Limites de caracteres**: `nome` da refeição e do bloco 120 (padrão igual a outros campos "nome"), item 200, `horario` 20, `observacoes` 500 (padrão igual a `descricao`).
