...
const MEDIDA_KEYS: (keyof BodyMetricMedidas)[] = [
  'cintura',
  'quadril',
  'peito',
  'pescoco',
...
/** Parses the 8 free-text measurement fields from the quick-add form into numbers — shared by
 * the save handler and the live body diagram, so both read the same "what did the user actually
 * type" logic instead of duplicating it. Blank or non-numeric text is simply omitted. */
export function parseMedidasText(text: Record<keyof BodyMetricMedidas, string>): BodyMetricMedidas {
